...
if [ $# != 1 ]; then
   ...
   echo "Usage: local/nnet/monolingual_dnn.sh L"
   echo "  where L in {MD, SW, HG, UR, AR, DT}"
   exit 1;
fi
L=$1
...
if [ $stage -le 0 ]; then
  ...
  steps/nnet/make_fmllr_feats.sh --nj $feats_nj --cmd "$train_cmd" \
     ...
     $dir data/$L/eval $gmmdir $dir/log $dir/data || exit 1
  # train
  dir=$data_fmllr/train
  steps/nnet/make_fmllr_feats.sh --nj $feats_nj --cmd "$train_cmd" \
     --transform-dir ${gmmdir}_ali \
     $dir data/$L/train $gmmdir $dir/log $dir/data || exit 1
...
if [ $stage -le 2 ]; then
  ...
  dir=exp/$L/dnn4_pretrain-dbn_dnn
  ali=${gmmdir}_ali
  feature_transform=exp/$L/dnn4_pretrain-dbn/final.feature_transform
  dbn=exp/$L/dnn4_pretrain-dbn/6.dbn
  ...
  steps/nnet/decode.sh --nj $decode_nj --cmd "$decode_cmd" --acwt 0.2 \
    $gmmdir/graph $data_fmllr/eval $dir/decode_eval || exit 1;
fi
