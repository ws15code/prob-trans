...
for x in train eval; do
	...
	case "$LCODE" in
		...
		SW)
			...
			;;
		UR)
			./local/sbs_create_phntrans_UR.sh $tmpdir/downsample/${x}_basenames_wav conf/${full_name}/g2pmap.txt conf/${full_name}/Urdu_numerals_to_graph_dict.txt $TRANSDIR/${full_name} local/tmp2/UR > $tmpdir/${LCODE}_${x}.trans
			;;
		*) 
	...
	# amit: Add some hacks
	join  <(cat $tmpdir/${LCODE}_${x}.trans | sort -k1,1 |awk '{ if ($2 != "") print $1;}') <(cat $tmpdir/${LCODE}_${x}.trans | sort -k1,1 ) | sed 's/ /\t/' > data/${LCODE}/local/data/${x}_text
	cat data/${LCODE}/local/data/${x}_text | cut -f1 > $tmpdir/downsample/${x}_basenames_wav
	paste $tmpdir/downsample/${x}_basenames_wav <(cat $tmpdir/downsample/${x}_basenames_wav|awk -v DIR="data/$LCODE/wav/$x" '{ print DIR"/"$1".wav"}') > data/${LCODE}/local/data/${x}_wav.scp	
	echo "${x}_wav.scp has $(cat data/${LCODE}/local/data/${x}_wav.scp|wc -l) files"
	# amit
